#!/bin/bash
# sweep.sh — Run iiLoss or EdLoss hyperparameter sweep (single-stage or PCGrad).
#
# Usage:
#   bash sweep.sh iiLoss             # iiLoss λ sweep,        single-stage
#   bash sweep.sh EdLoss             # EdLoss α sweep,        single-stage
#   bash sweep.sh iiLoss pcgrad      # iiLoss+PCGrad,         two-stage
#   bash sweep.sh EdLoss pcgrad      # EdLoss+PCGrad,         two-stage
#
# Optional third argument overrides the training target (default: Ef):
#   bash sweep.sh iiLoss single Ed
#
# Prerequisites:
#   - conda environment 'mlstab' (see environment.yml)
#   - GPU recommended; set CUDA_VISIBLE_DEVICES="" to force CPU
#   - For pcgrad runs: stage-1 checkpoints are generated automatically

# ── Arguments ─────────────────────────────────────────────────────────────────
MODEL_TYPE=${1:-iiLoss}   # iiLoss | EdLoss
MODE=${2:-single}         # single | pcgrad
TARGET=${3:-Ef}           # Ef | Ed

if [[ "$MODEL_TYPE" != "iiLoss" && "$MODEL_TYPE" != "EdLoss" ]]; then
    echo "ERROR: MODEL_TYPE must be 'iiLoss' or 'EdLoss', got '$MODEL_TYPE'"
    exit 1
fi
...
if [[ "$MODE" == "pcgrad" ]]; then
    ...
    if $NEED_STAGE1; then
        ...
        $PYTHON -u mlstabilitytest/train_models.py allMP_2026 "$TARGET" iiLoss_save_0.0 \
            2>&1 | tee -a "$LOGFILE"
...
# ── Main sweep ─────────────────────────────────────────────────────────────────
SPLIT=$( [[ "$MODE" == "pcgrad" ]] && echo "allMP_2026_finetune" || echo "allMP_2026" )
N=${#VALUES[@]}
...
for i in "${!VALUES[@]}"; do
    ...
    $PYTHON -u mlstabilitytest/train_models.py "$SPLIT" "$TARGET" "$MODEL" \
        2>&1 | tee -a "$LOGFILE"
...
echo "    python interference_score.py allMP_2026"             | tee -a "$LOGFILE"
echo "============================================================" | tee -a "$LOGFILE"
